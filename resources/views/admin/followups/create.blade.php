...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} Follow Up
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.followups.store")  }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select {!! $contactId ? 'readonly' : '' !!} class="form-control select2 {{ $errors->has('contact') ? 'is-invalid' : '' }}" name="contact_id" id="contact_id">
                    @foreach($contacts as $id => $entry)
                        <option value="{{ $id }}" {{ old('contact_id') == $id || $contactId == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
